    ...
    public static String substring(String src, int start, int length, String ov) {
        if (src != null && length(src) > length) {
            try {
                ...
                for (byte c : bs)
                    if (c < 0) count++;
                if (count % charlen != 0) return substring(src, start, length - (count % charlen), ov);
...
//	}
    ...
    public static String native2ascii(String str) {
        ...
        for (int x = 0; x < ca.length; ++x) {
            char a = ca[x];
            if (a > 255) buffer.append("\\u").append(Integer.toHexString(a));
            else {
                buffer.append(a);
            }
        }
    ...
    public static String getPartString(String str, int len, String showStr) throws UnsupportedEncodingException {
        ...
        if (counterOfDoubleByte % 2 == 0) return new String(b, 0, len, "UTF-8") + showStr;
        else return new String(b, 0, len - 1, "UTF-8") + showStr;
    }
    ...
    public static String ListToStr(List<String> list, String regex) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i < list.size() - 1) sb.append(regex);
        }
        return sb.toString();
    }
    ...
    public static String ArrToStr(Object[] strArr, String regex) {
        ...
        for (int i = 0; i < strArr.length; i++) {
            sb.append(strArr[i]);
            if (i < strArr.length - 1) sb.append(regex);
        }
        return sb.toString();
    }
    ...
    public static String encode(String str, String enc) {
        String enstr = "";
        if (str == null || str.equals("")) return enstr;
        try {
