...
public class OrderServiceImpl implements IOrderService {
    ...
    public Order pay(int id) {
        ...
        Message message = MessageBuilder.withPayload(OrderStatusChangeEvent.PAYED).setHeader(
                "order", order).build();
    ...
    public Order deliver(int id) {
        ...
        if (!sendEvent(MessageBuilder.withPayload(OrderStatusChangeEvent.DELIVERY).setHeader(
                "order", order).build(), orders.get(id))) {
    ...
    public Order receive(int id) {
        ...
        if (!sendEvent(MessageBuilder.withPayload(OrderStatusChangeEvent.RECEIVED).setHeader(
                "order", order).build(), orders.get(id))) {
            System.out.println("线程名称:" + Thread.currentThread().getName() + " 收货失败, 状态异常, 订单号:" + id);
        }
        return orders.get(id);
